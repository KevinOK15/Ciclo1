...
public class Mochila extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		if (modelo == "Sherman") {prec=149.9;}
		else if (modelo == "Faguo") {prec=89.9;}
		else if (modelo == "Aldo") {prec=119.9;}
		else {prec=174.9;}
		impC = prec*cant;
		...
		if (impP<200){choc=1*cant;}
		else if (impP>=200 && impP<500){choc=2*cant;}
		else if (impP>=500 && impP<700){choc=8*cant;}
		else {choc=10*cant;}
		
		txtS.setText("SISTEMA DE MOCHILA \n\n");
		txtS.append("Importe compra :" + decimalFormat(impC) + "\n");
		txtS.append("Importe descuento :" + decimalFormat(impD) + "\n");
		txtS.append("Importe a pagar :" + decimalFormat(impP) + "\n");
		txtS.append("Chocolates :" + choc + "\n");
	}
